...
message::~message() try {
    ...
    if (its_configuration->has_console_log()
        ...
        if (its_configuration->has_console_log()) {
            ...
            switch (level_) {
            case level_e::LL_FATAL:
                std::cout << "[FATAL][" << app << "] VSIP: " << buffer_.data_.str() << std::endl;
                break;
            case level_e::LL_ERROR:
                std::cout << "[ERROR][" << app << "] VSIP: " << buffer_.data_.str() << std::endl;
                break;
            case level_e::LL_WARNING:
                std::cout << "[WARN ][" << app << "] VSIP: " << buffer_.data_.str() << std::endl;
                break;
            case level_e::LL_INFO:
                std::cout << "[INFO ][" << app << "] VSIP: " << buffer_.data_.str() << std::endl;
                break;
            case level_e::LL_DEBUG:
                std::cout << "[DEBUG][" << app << "] VSIP: " << buffer_.data_.str() << std::endl;
                break;
            case level_e::LL_VERBOSE:
                std::cout << "[TRACE][" << app << "] VSIP: " << buffer_.data_.str() << std::endl;
                break;
            default:
                std::cout << "[INFO ][" << app << "] VSIP: " << buffer_.data_.str() << std::endl;
            };
